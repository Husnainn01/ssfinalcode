...
export default function PurchaseAgreementsPage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      setIsLoading(true)
      try {
        const response = await fetch('/api/admin/vehicles/documents?type=purchase_agreement')
        if (!response.ok) {
          throw new Error('Failed to fetch purchase agreement documents')
        }
        const data = await response.json()
        setDocuments(data.documents)
      } catch (err) {
        setError(err.message)
        console.error('Error fetching purchase agreement documents:', err)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this purchase agreement document?")) {
      try {
  ...
  const handleUpload = async () => {
    if (!uploadingFile || !uploadingVehicleId) {
      setUploadError("Please select a vehicle and file")
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to upload purchase agreement')
      }
      ...
      console.error('Error uploading purchase agreement:', err)
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">Purchase Agreement Documents</h1>
        </div>
        ...
        <Dialog>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>Upload Purchase Agreement</DialogTitle>
              <DialogDescription>
                Upload a purchase agreement for a customer vehicle
              </DialogDescription>
            </DialogHeader>
      ...
      <div className="flex items-center space-x-2">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search purchase agreements..."
            value={searchTerm}
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded flex items-center">
          ...
          <span>Error loading purchase agreement documents: {error}</span>
        </div>
      )}
      ...
      {isLoading ? (
        <div className="flex justify-center items-center py-8">
          ...
          <span className="ml-2">Loading purchase agreement documents...</span>
        </div>
      ) : (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle>Purchase Agreement Documents</CardTitle>
          </CardHeader>
                ...
                {filteredDocuments.length > 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-gray-500">
                      No purchase agreement documents found
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
