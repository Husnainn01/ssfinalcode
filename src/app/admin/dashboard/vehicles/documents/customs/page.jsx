...
export default function CustomsDocumentsPage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      setIsLoading(true)
      try {
        const response = await fetch('/api/admin/vehicles/documents?type=customs_document')
        if (!response.ok) {
          throw new Error('Failed to fetch customs documents')
        }
        const data = await response.json()
        setDocuments(data.documents)
      } catch (err) {
        setError(err.message)
        console.error('Error fetching customs documents:', err)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this customs document?")) {
      try {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      formData.append('type', 'customs_document')
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to upload customs document')
      }
      ...
      console.error('Error uploading customs document:', err)
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">Customs Documents</h1>
        </div>
        ...
        <Dialog>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Upload a customs document for a customer vehicle
              </DialogDescription>
            </DialogHeader>
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded flex items-center">
          ...
          <span>Error loading customs documents: {error}</span>
        </div>
      )}
      ...
      {isLoading ? (
        <div className="flex justify-center items-center py-8">
          ...
          <span className="ml-2">Loading customs documents...</span>
        </div>
      ) : (
        <Card>
